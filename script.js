            ...
            entries.forEach(entry => {
                ...
                if (entry.isIntersecting) {
                    ...
                    if (desktopVideo.src !== newVideoSrc) {
                        ...
                        desktopVideo.load(); 
                        desktopVideo.play().catch(error => console.log("Autoplay was prevented:", error));
                    }
                }
            });
        };
...
function calculateCurrentCost() {
    ...
    const savingsFromFalsePositives_number = ((searchVolume * 0.4 * 0.9 * 10) / 60) * 29;
    ...
    const applicablePrice = (0.50 * currentCost > ourPrice) ? 0.50 * currentCost : ourPrice;

    const estimatedAmlCost_number = Math.floor(totalCost - (searchVolume * applicablePrice)) + (savingsFromFalsePositives_number * 0.44);

   
    
    currentAMLCostDiv.textContent = totalCost.toLocaleString('en-US');
    savingsFromFalsePositivesDiv.textContent = savingsFromFalsePositives_number.toLocaleString('en-US');
    annualReductionDiv.textContent = annualReduction_number.toLocaleString('en-US');
    
   
    estimatedAmlFinalDiv.textContent = parseInt(estimatedAmlCost_number).toLocaleString('en-US');
